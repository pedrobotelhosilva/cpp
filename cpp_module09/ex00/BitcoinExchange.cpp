...
void BitcoinExchange::fillData(std::string line)
{
    ...
    while (getline(ss, token, ','))
    {
        if (i == 0)
        {
            this->LineData.date = token;
            i = 1;
        }
        else
            this->LineData.value = atof(token.c_str());
    }
    this->btcData.insert(std::make_pair(this->LineData.date, this->LineData.value));
}
...
void BitcoinExchange::handleInputFile(std::string fileName)
{
    ...
    while (getline(file, line))
    {
        ...
        pos = line.find(' ');
        this->LineData.date = line.substr(0, pos);
...
void BitcoinExchange::findDateAndCalculate()
{
    ...
    it = this->btcData.lower_bound(this->LineData.date);
    if (it == this->btcData.end())
    {
        it--;
        std::cout << std::setprecision(7) << this->LineData.date << " => " << std::setprecision(7) << this->LineData.value << " = " << this->LineData.value * (*it).second << std::endl;
        return;
    }
    ...
    {
        if ((*it).first != this->LineData.date)
            it--;
        std::cout << std::setprecision(7) << this->LineData.date << " => " << std::setprecision(7) << this->LineData.value << " = " << std::setprecision(7) << (float)(this->LineData.value * (*it).second) << std::endl;
    }
}
